...
T["build"]["file"]["should include files"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -f init.lua",
    [[--ignore-case 'require' -g '{init.lua}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -f init.lua README.md *.test.*",
    [[--ignore-case 'require' -g '{init.lua,README.md,*.test.*}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -f init.lua -f README.md -f *.test.*",
    [[--ignore-case 'require' -g '{init.lua,README.md,*.test.*}']]
  )
end
...
T["build"]["file"]["should include and exclude files"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -f README.md !init.lua",
    [[--ignore-case 'require' -g '{README.md}' -g !'{init.lua}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -f README.md -f !init.lua",
    [[--ignore-case 'require' -g '{README.md}' -g !'{init.lua}']]
  )
end
...
T["build"]["extensions"]["should include extensions"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -e lua",
    [[--ignore-case 'require' -g '{*.lua}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -e lua rb md",
    [[--ignore-case 'require' -g '{*.lua,*.rb,*.md}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -e lua -e rb -e md",
    [[--ignore-case 'require' -g '{*.lua,*.rb,*.md}']]
  )
end
...
T["build"]["extensions"]["should include and exclude extensions"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -e md !lua",
    [[--ignore-case 'require' -g '{*.md}' -g !'{*.lua}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -e md -e !lua",
    [[--ignore-case 'require' -g '{*.md}' -g !'{*.lua}']]
  )
end
...
T["build"]["dirs"]["should include dirs"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -d plugins",
    [[--ignore-case 'require' -g '{**/plugins/**}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -d plugins feature_*",
    [[--ignore-case 'require' -g '{**/plugins/**,**/feature_*/**}']]
  )
  expect.equality(
    rg_pattern_builder.build "~require~ -d plugins -d feature_*",
    [[--ignore-case 'require' -g '{**/plugins/**,**/feature_*/**}']]
  )
end
...
T["kitchen sink"] = function()
  expect.equality(
    rg_pattern_builder.build "~require~ -e rb md !lua -d plugins !feature_* -f !*.test.* *_spec.rb",
